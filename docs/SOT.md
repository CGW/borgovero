...
**Last updated:** 2026-09-01 (S010 — the session that ran what S009 built,
and found that three of the things the site was publishing were still ours
rather than the market's. **All of S009's queued work is now RUN, BUILT,
DEPLOYED and COMMITTED** (`7b9854d`): 1.386 of 1.388 listing photographs on
disk, all five build gates passing, casazebra.it serving the S010 tree,
verified with cache-busted requests against two URLs that exist only in
this build. Three bugs, each invisible in the way this project keeps
finding: **im-cdn retired the `large` size name**, 403 for every photo id,
which would have published the 844-listing immobiliare block — the largest
source — with no photograph at all; `photomatch` uses `thumb`, which still
works, which is why 12.203 hashes never surfaced it. **Cross-agency `ref`
was still forming clusters** in `contradictions.cluster()` although §16d
settled in S009 that it is decisive only within one agency; six ref-only
rows were live as market disagreements, and **the worst price gap on the
site was 94% when the real one is 26%**. **A reference number was being
used as an address label** — Lancisi rows carry `address_raw` NULL and the
title `Rif. 3890` — which the S009 address gate then refused, correctly by
its own rule and for the wrong reason. Still open and untouched: Torre di
Berta, the six WordPress detail parsers, NOW's 47 NULL comuni, the
`cerca.json` `\d{6,}` bug, and the legal hour.)

**Superseded, kept for the record — 2026-09-01 (S009 — the session that turned the agency
sites into listings, and found three published claims that were ours
rather than the market's. **`promote_agency_sites.py`**: the eight agency
sites left the `agency_site_listings` side table, where they were a
`url_alt` mapping invisible to every matcher — **1.295 → 1.917 listings,
936 → 1.399 publishable**, no new fetching. **`harvest_agency_details.py`**:
588 of 622 got photographs, 516 (83%) with the 2+ distinct images a merge
needs. **`discover_agency_urls.py`**: sitemap enumeration, and Romolini's
own sitemap holds **1.038 listing-shaped URLs against 40 harvested** —
including ref 1594, the orphan that started the session. Corrected here:
**§16b's photo-harvest state was four sessions stale** (complete, not 6%),
and **§16d's `ref` route is decisive only WITHIN one agency** — across
agencies it produced 9 hits, all 9 different properties. Bugs that were
publishing our own errors: the `badia` alias filed **Badia Petroia
(Umbria)** under Badia Tedalda, and §16d publishes comune conflicts as
findings; `best_label()`'s `max(key=len)` **preferred the civico**, and two
pages were live with a house number and a CAP in the URL. **Addresses are
now capped at street name, no house numbers ever**, with a build gate.
**Listing photographs are published, credited in 8pt, EXIF stripped,
removed on request** — a deliberate position, taken after generative art
and a sepia filter were considered and rejected. **NOT YET RUN: the image
harvest is partway (146 of ~1.388), `photomatch --harvest` has not run
against the new agency photos, and nothing is rebuilt or deployed — the
live site still serves the S008 build, including the two civico URLs.**
[S010: all of that is now run and deployed — see the header above. The two
civico URLs are gone.]
Torre di Berta remains **unresolved**: Romolini 3018 is €215.000/170 m²
and in the corpus, 1594 is €140.000/171 m² and still is not.)

**Superseded, kept for the record — 2026-08-31 (S008 — the dossier session: the
agency-site link harvester ran across **all nine agencies Christopher
named** (234 portal rows now carry their own-site URL in `url_alt`,
tracked as `phase0/data/url_alt.json`); the candidate-pair
deep photo pass hashed 440 full-gallery images and Centogambe
0674 ↔ Marcellini 10721 is photo-strong; listing pages grew the S008
dossier blocks (own-site channel + candidate leads under §16d
discipline). **All of it is APPLIED, DEPLOYED and INDEXED**: the
typology fix and harvest data are in the real database, casazebra.it
serves the S008 build, and Search Console reports the sitemap
**Success, 1.630 pages discovered** (2026-08-31). Also: **the queued
exhibit died honorably** — the
"portal says appartamento" half of the Leonardi cielo-terra case was
OUR normalization artifact (caption overriding the portal's structured
field on 45 rows), fixed in `map_typology` + `apply_S008_typology.py`,
which moves 22 tiers and nudges all eight comune bands. Late in the
session the front door changed shape: `/it/comuni/` is now **a table,
one row per comune, chiesto against venduto (OMI)**, both quoted on the
same surface basis. **Next: the §16d eyeball queue — the €149.000 pair,
the Via Tiberina 4-way, the Centogambe 0032 pair — then S009.**)
...
| S008 | 2026-08-31 | **`/it/comuni/` answers the buyer's question: chiesto vs venduto, one row per comune.** Shipped as a TABLE (the card grid it replaced read one comune at a time; eight aligned rows read against each other, which is the comparison the page exists for). Columns: comune, annunci, **Chiesto al m²**, **Venduto al m²**, and a worded cue. The unit lives in the column head and the two provenance notes are CSS-only `?` popovers on the heads — sixteen repetitions of "/m²" and sixteen `?` icons are what stopped it reading as a table; they open on hover AND keyboard focus, keep their text in the HTML, and hide in print. `Venduto` is coloured against `Chiesto`: **red** where the sold band sits entirely below it (Citerna alone: €884 asked, €350–780 sold), **amber** where it straddles (five comuni), **green** where it does not sit below (Badia Tedalda €411 vs €600–1.200, Pieve Santo Stefano €570 vs €670–1.250). Colour never carries the meaning alone — a worded cue sits beside it, because colour-blind and printed readers would otherwise lose the signal and because on a site that names agencies a bare red number reads as an accusation rather than a comparison. Christopher asked for a per-comune row showing asking price, a single ask-to-close percentage, and an implied sale price. Two of the three could not be published and the data said why, which is the useful part. (1) A single "€1.870 (mediana)" for Anghiari is **exactly the midpoint** of its €1.708,8–2.031,1 band — the one operation `normalize.py`'s header forbids by name. (2) A "~35% sopra il prezzo di vendita" cannot be computed: `price_history` holds 173 rows from two timestamps and **zero observed price changes**, so the ladder S003 started accumulating has produced nothing yet, and the only other sources are the OMI gap or `DOM_DISCOUNT`, whose own comments say every rung above the 8% national floor "is an extrapolation with no measurement behind it". A per-property ask-to-close estimate would additionally be a *stima* of a named agency's named listing (§3.5). (3) **The surface-basis trap nearly repeated §7 verbatim**: OMI quotes on LORDA, our normalized €/m² uses internal habitable area, and comparing them books a definitional gap as a market one — Anghiari's tight end moves **19% → −12%** on that swap alone, and the naive cross-paired version reads +4%…+154%. So `Chiesto` is a median of published asking prices over the agencies' own stated surface — a description of what is advertised, not an index figure and not a midpoint of anything — and `Venduto` is the OMI band on that same basis. The two inversions mean the S006 "below the OMI floor" finding now sits on the front door instead of being averaged away by a discount column. `OMI_SEMESTER`/`OMI_RESIDENTIAL` exclude Box/Magazzini/Negozi. Note the join: **`omi_bands.comune` is normalized (`badiatedalda`) while listings carry the hyphenated slug** — matching on the slug returns nothing for three of eight comuni and looks precisely like "OMI has no band here". |
| S008 | 2026-08-30 | **A third build gate: the sitemap must be deployable.** Caught before deploy, one command short of shipping it: `build.py --base-url` was documented as "REQUIRED for any build that will be deployed" and **defaulted to empty**, so the deploy command handed over without it produced a tree whose `<loc>` entries were path-only (`/dati/`) and whose robots.txt read `Sitemap: /sitemap.xml`. Search Console rejects path-only `<loc>`, so deploying would have flipped the sitemap report from Success to errors — and silently, since nobody opens GSC for days. Two fixes, because the warning alone was the thing that failed: the production origin is now `PROD_ORIGIN` and the **default**, and a third gate refuses to install any tree whose sitemap carries a non-`http` `<loc>` (announcing "sitemap: 1.630 absolute URLs under https://casazebra.it" when it passes). `--base-url ''` still works for a local preview and says out loud that the tree must not be deployed. The lesson is the S005 lint's, in a new place: **a required argument that is silently optional is not a safeguard**, and the gate belongs where the artifact is produced, not in the instructions for producing it. |
| S008 | 2026-08-30 | **The deploy step was undocumented, and it has a trap.** Three sessions of "deployed to Vercel" and nowhere did it say HOW. Established from the Vercel API: production is the **non-git-connected project `dist-site`** (`prj_kzhgio7otE8l2Ec75on36d6bWIrj`, team `cgw-2712s-projects`), deployed by CLI from `bv-site/dist-site` — which is why the project carries the folder's name. The trap: **`build.py` installs via `shutil.rmtree()` on its output directory, so every build deletes `dist-site/.vercel`**. A bare `vercel --prod` afterwards finds no link, offers to create a project, and on the default answer deploys to a NEW project no domain points at — a green "success" that changes nothing live. Recorded in CLAUDE.md with the explicit `vercel link --yes --project dist-site` line. Same family as the `id_anchors.json` folklore and the "missing" OMI files: **a step nobody wrote down gets re-derived under pressure, and the re-derivation is where it goes wrong.** |
| S008 | 2026-08-30 | **The harvester covers all nine agencies (second wave).** Christopher supplied the full Sansepolcro agency list, so the four-site brief was extended the same session: **NOW 6/36, SICASA 18/35, ImmobilInvest 6/11, Lancisi 0 (by construction), Tiber not harvestable.** Total **234 of 466 portal rows** carry `url_alt`; 1.165 index cards held across eight sites. Three findings came out of the access checks, and each is a fact about the agency, not a parser problem: **Tiber Immobiliare has NO own website** (38 portal rows — Facebook and portals only), recorded in `NO_OWN_SITE` so nobody looks again; **NOW's archives render client-side** (`/properties/` and `/citt/` serve ~117 KB of chrome and zero listings, the estate sitemap holds one loc), so only its homepage's 50 cards are harvestable and rendering JS to reach the rest is the line already declined at Immobiliare's detail pages and Idealista (§9/§12.4); and **Lancisi publishes 68 listings on its own site and has ZERO portal rows** — the S004-era "establish why" question is answered: it is not absent from the market, it is absent from the portal, which is a §16e channel-opacity observation and not a gap in our corpus. Parsing lesson worth keeping: these WordPress themes render one card as two or three anchors to the same listing, and the figures do not all sit behind the same one (House's behind the last, SICASA split across two, NOW's behind the MIDDLE with empty windows either side). Picking one occurrence is a guess that fails SILENTLY — NOW first harvested 50 cards and 0 prices — so `_merge_windows()` parses every window a link opens and merges field-by-field; decoy windows contribute nothing and cost one regex pass over text already in memory. ImmobilInvest is static HTML with listings INLINE on one page (no per-listing URLs, so `url_alt` points at the catalog and the Rif is the identity) and a charset mismatch that delivers `€` as `â\\x82¬` — worth 6 matches once decoded. |
| S008 | 2026-08-30 | **The harvester ran, the deep pass ran, the dossier shipped — and the session's queued exhibit was our own artifact.** `adapters/agency_sites.py` + `harvest_links.py`: index-only harvest of the four portal agencies' own sites (robots.txt checked per site first; Romolini's forbids query-param pagination, so its coverage is partial by design at 33/93; Cortesi's cards carry no surface, so 23 unique candidates each earned ONE detail fetch under §15's exception clause). **202 of 466 portal rows now carry `url_alt`** — Leonardi 65/159 (incl. the proven 105891071 ↔ rif. 4064 case), House 76/127, Romolini 33/93, Cortesi 28/87 — matched by price+surface+comune under the no-coin-flip rule (ambiguous groups drop; a unique-exact-surface assignment inside a group is allowed, because surfaces of 211↔211 next to 240↔240 are the surface discriminating, not a coin flip). Ref route reports itself idle: every portal row carries `agency_ref` NULL. Tracked exports `data/url_alt.json` + `data/agency_site_index.json`; the build consumes the JSON, never the network. **The exhibit correction:** the brief's "portal says appartamento, own site says cielo terra" for 105891071 was false — Immobiliare's structured field says *Terratetto unifamiliare*, agreeing with the agency site; the "appartamento" came from `map_typology()` scanning field+caption as one blob in dict order, letting agency headlines override the portal's field on **45 of 844 rows**. Fixed (field first, caption as fallback; `apply_S008_typology.py`, idempotent): 22 tier changes, all eight comune bands move, 45 listing slugs churn (no archive layer — the §6 cost again), Badia Tedalda stays below the OMI floor. What survives of the exhibit is a self-contradiction: the agency's own headline against both channels' typology. **Deep photo pass** (`deep_photo_pass.py`): full galleries for 26 candidate-cluster listings only (440 images; PHOTOS_PER_LISTING untouched per §10.1); photomatch's own accounting now has Centogambe 0674 ↔ Marcellini 10721 photo-strong (3 distinct shared images ≤5) inside the Via Tiberina cluster, which correctly still prints as candidate — weak edges bridge the two strong pairs, and the overlay demotes a merge that exceeds the verified member set. Listing pages gained the dossier blocks: own-site channel labelled by match route ("il collegamento è un dato, non un accertamento"), candidate clusters as leads under the §16d unconfirmed discipline; `cerca.json` resolves pasted agency-site URLs (1.360 entries). Build: twice byte-identical, lint clean, 1.641 files. **Parser count is now 7** (immobiliare, marcellini, centogambe + four site indices) — each one weekly maintenance forever; the heartbeat argument writes itself. |
| S009 | 2026-09-01 | **The agency sites became listings, and three published claims turned out to be ours.** Started from one URL Christopher pasted — Romolini ref 1594, €140.000, 171 m², live with 2.955 views — that the search box answered "no matching listing in our archive" about, correctly: it had never been seen. S008 walked `/it/comune/<comune>.php` because robots forbids query-param pagination, and **1594 is not linked from that index**. An index built by following a site's menus inherits that site's decisions about what to show. **`promote_agency_sites.py`** moved the eight agency sites out of the `agency_site_listings` side table, where they were only a `url_alt` mapping and were invisible to every matcher: **1.295 → 1.917 listings, 936 → 1.399 publishable**, no new network traffic, idempotent, zero fabricated `price_history` rows. Honest number for the gain is **463 newly publishable**, not the 834 first quoted — that count ignored comune scope, and Leonardi contributes 267 of 692 because 425 are Città di Castello, San Giustino, Umbertide, Magione. **`harvest_agency_details.py`**: detail-page pass, 588 of 622 got photo URLs, 12 surfaces recovered, probed against all eight live sites first — which caught five extraction defects, none self-announcing. Worst: **ImmobilInvest sells inline on one catalog page and the `#rif-244` fragment in `url_alt` is OURS**, synthesised by S008; the page has no such anchor, so every listing read the whole page and refs 244 and 369 got the SAME eight photographs — eighteen listings sharing eight images each, the transitive merge §16d already records. Segmented on the printed `Rif` text instead: 18/18 with photos, 0 shared. Also: WordPress serves `1-1-154x154.jpg` beside `1-1.jpg` and **dHash is resolution-independent but NOT aspect-independent**, so a square crop hashed against a 4:3 original is a false negative that reads as honest absence. **`discover_agency_urls.py`**: seven of eight sites publish sitemaps (Romolini's is advertised in their own robots.txt — 8.626 locs, 1.038 listing-shaped against 40 harvested, **1594 confirmed present**); its Romolini detail parser verified on three live pages, after two regexes in it were found **dead on arrival** — each ended a capture on `\s{2,}` after the line above collapsed all whitespace to single spaces, so neither could ever match, and the breadcrumb one would have silently discarded all 888 listings whose slug names no comune. **A regex that cannot match is indistinguishable from a site that does not publish the field.** |
| S009 | 2026-09-01 | **Three things the site was publishing were our errors, not the market's.** (1) **`_COMUNE_ALIASES` carried a bare needle `badia`**, matched with `startswith()`, filing four Cortesi listings from **Badia Petroia** — a frazione of Città di Castello, in Umbria, in Perugia province, under its own OMI order — as **Badia Tedalda**, which held fifteen agency rows in total. Since §16d *publishes comune conflicts as a finding*, the bug would have printed two correctly-filing agencies as contradicting each other, in public, on a page carrying a right-of-reply button. Same shape as the `rif` regex that matched inside pe·**rif**·eria. Fixed in the alias table + `fix_S009_badia.py` (idempotent). (2) **`contradictions.best_label()` chose the cluster address with `max(addrs, key=len)`** — the longest address is by construction the one carrying the civico and the CAP, so the rule actively preferred the most identifying string available. Two pages were **live** with it in the URL and title: `sansepolcro-via-della-ginestra-4-cd4560` and `sansepolcro-via-dei-tarlati-52037-cb6e9e`. Christopher's call: **street name is the ceiling, no house numbers ever.** New `phase0/address_privacy.py` (17/17 cases, incl. localities and CAPs mistaken for civici), sanitising *before* the max so longest now means most informative among street names; a civico-only difference no longer prints as a location disagreement, because two agencies naming the same street is agreement. (3) **`photomatch.DELAY_S = 0.4` is a statement about `pic.im-cdn.it`, not about images** — it was chosen when every hashed image was a 2,7 KB CDN thumbnail. The eight agency sites serve originals off their own hosting, **measured before the constant was reused on them**: median ~259 KB, ~96× the thumbnail, 3.773 of them ≈ 0,93 GB, and at 0.4s a sustained ~650 KB/s aimed at small-business WordPress hosts. `agencies.py` sets the politeness doctrine for those sites and this would have broken it quietly, **because the delay lives in a different file from the doctrine**. `delay_for(url)`: CDN keeps 0.4s, agency hosts get 2.0s. *A constant carries the assumptions of whatever it was measured against; reusing it on a different kind of host is a new decision, not an inherited one.* |
| S009 | 2026-09-01 | **Listing photographs published, deliberately, with the mitigations that make it defensible.** Considered and rejected: generative art from a 179-byte palette+grid signature (safe, but does not show the property — four treatments rendered and compared before Christopher ruled it out) and a **sepia filter**, which changes nothing about copyright or personal data and reads as disguise rather than good faith. Position taken: **publish the agency's photo, credited, linked, removed on request.** Self-hosted rather than hotlinked — hotlinking spends their bandwidth on our traffic, breaks when they reorganise, and leaves us unable to strip anything. `harvest_listing_images.py`: 600px WebP q70, and **re-encoding without `exif=` is what drops GPS, camera serial and original timestamp**. 600px reduces the odds a face or a plate is legible; it does not eliminate them, and the honest description is *we take it down when asked*, not *we made it safe*. `listing_image()` renders the figure with an 8pt caption naming and linking the agency; the opt-out list is consulted at fetch **and** at render, because a takedown depending on someone editing a template is not a takedown. **Two new build gates, both because the failure is invisible**: a civico in a slug looks exactly like a slug, and a page with an uncredited photo looks completely fine. Tested against the live tree — the address gate flags exactly the two pages that were serving. Note on GDPR: the checklist Christopher surfaced (staging out family photos, blurring plates) is written for the party **holding the camera** and is not actionable for a downstream index; the material question for CasaZebra is not the photos but that it publishes street, price, surface, DOM and price-cut history as a permanent archive that outlives the listing — worth an hour of Italian legal advice, the same hour that answers art. 70 comma 1-bis. Also: **`--optout` shipped with no inverse and was fat-fingered within four minutes of handover** (no harm — Romolini's images had not been fetched yet). `--allow` added. *A destructive command with no inverse is a trap, and the inverse being cheap is why it was easy to forget to write.* |
| S010 | 2026-09-01 | **Ran what S009 built, and three more published claims were ours.** All of S009's queued work is now run, built, deployed and committed (`7b9854d`); the live site is the S010 tree, verified with cache-busted requests against two URLs that exist only in this build — the relabelled `caprese-michelangelo-16affb.html` and an immobiliare photo. **(1) im-cdn retired the `large` size name.** `pic.im-cdn.it/image/{id}/large.jpg` returns **403 Bad Parameters for every photo id**, including ids that serve fine at other names — so not our ids, not a rate limit, not the network. 784 immobiliare listings, **the largest source at 844**, were about to publish with no photograph. Surviving names are `xxl`/`xl`/`l`/`xs-c` at 599×450 and `xxs-c` at 320×240; **`photomatch` uses `thumb`, which still works, which is why 12.203 hashes never surfaced it.** Diagnosed from `cache/html`, where the ids appear as `pwm.im-cdn.it/image/379238631/xxs-c.jpg`. `xxl` is one pixel under our own `WIDTH = 600`, so the resize is now a no-op for immobiliare; EXIF still drops on re-encode. **1.386 of 1.388 on disk**, the 2 failures dead files on the agencies' own hosts. *A dead URL template and an empty result look identical from inside the loop — the same shape as S009's regex that could not match.* **(2) Cross-agency `ref` was still forming clusters.** §16d settled in S009 that ref is decisive only WITHIN one agency — across agencies it produced 9 hits, all 9 different properties — and **`contradictions.cluster()` never got the memo**: `by_ref` keys on `(comune, ref)` and fires the moment two agencies share one. Six ref-only rows were live as market disagreements. **Rif. 4478 paired a Lancisi 90 m² flat with a Marcellini 7.000 m² capannone and printed "7678% apart".** The mechanism is dull: several agencies number stock as a zero-padded 4-digit sequence, `lstrip("0")` makes `0831` and `831` the same string, and two agencies reaching their 831st listing in one comune is a collision. `ref` is now applied last and only to a cluster another route already formed. **Not one of the nine cross-agency ref groups coincided with a real cluster** — §16d confirmed on its own terms rather than asserted. What it costs is the point: **worst price disagreement 94% → 26%**, median 9% → 7%; worst surface 7678% → 200%; 112 → 105 properties. The headline number on the site was more than three times the real one. **(3) A reference number is not a label.** Lancisi publishes rows with `address_raw` NULL and the whole title `Rif. 3890`; `best_label()` fell through to the title and `slug()` produced `caprese-michelangelo-rif-3890-16affb.html`, which the S009 address gate refused — **correctly by its own rule and for the wrong reason: it cannot tell a house number from a filing number.** Loosening the gate to allow `rif` was the tempting fix and the wrong one; **a gate that learns to ignore a shape is how the civico got live the first time.** The label is fixed instead, the gate still refuses every trailing digit, and clusters with no street and no real title are now comune + hash. Some confronti URLs changed and the old ones 404 — deliberate, same as the civico URLs. Also: **`bv-site/assets/listing/` is now gitignored.** `git add -A` would have committed 1.386 third-party photographs, 49 MB, and **git history would have quietly made "removed on request" false** — `--optout` can delete from disk, but not from a pushed history. The images regenerate in 25 minutes. *An opt-out that cannot actually remove the file is the S009 template lesson one layer down.* Untouched and still open: Torre di Berta, the six WordPress detail parsers, NOW's 47 NULL comuni, the `cerca.json` `\d{6,}` bug, and the legal hour. |
...
### S008 OUTCOME (2026-08-30) — read before the brief below

The brief below ran and is DONE except the human looks. Corrections to
it, measured in S008: **its exhibit was our artifact** — the portal's
structured field for 105891071 says *Terratetto unifamiliare*, agreeing
with the agency site's *cielo terra*; "appartamento" was
`map_typology()` letting the agency's caption override the portal's
field (45 rows, fixed, see the S008 changelog row). What survives is a
self-contradiction (agency headline vs both channels), which is
`self_contradictions.py`'s axis. The harvester's numbers, all nine
agencies: **234/466 matched** — Leonardi 65/159, House 78/127,
Romolini 33/93, Cortesi 28/87, SICASA 18/35, NOW 6/36,
ImmobilInvest 6/11, Lancisi 0/0, Tiber no site. Ref route idle
everywhere (portal `agency_ref` all NULL). Romolini partial because
robots forbids query-param pagination; NOW partial because its
archives render client-side.
Open for Christopher's machine: `apply_S008_typology.py`,
`apply_S008_data.sql`, then deploy; and the §16d eyeball queue —
the €149.000 pair, the Via Tiberina 4-way expansion (Centogambe 0674 ↔
Marcellini 10721 now photo-strong), and the Centogambe 0032 ↔
Marcellini 10444 photo-weak pair (90 vs 170 m²).
...
**State (CORRECTED S009 — this paragraph was stale for four sessions).**
The harvest is **COMPLETE for the three ingested sources**: 1.271 listings,
**9.244 thumbnails** — immobiliare 841/6.894, marcellini 257/1.250,
centogambe 173/1.100. It previously read "52 of 841 hashed here … full
harvest ~45 minutes, should be run on Christopher's machine", and S009
quoted that back to him as the session's blocking problem before opening
the database. It had already been run. The cost of checking was one query;
the cost of not checking was recommending 45 minutes of finished work.

**The real gap is the eight agency SITES**, promoted to `listings` in S009.
`agency_site_listings` has no photo column at all — S008 read index cards,
never detail pages — so those rows entered the corpus *outside the photo
match space entirely*. That mattered more than it looked: photo is the only
route left to a PRICE disagreement once §16d's `ref` correction below is
applied. `harvest_agency_details.py` closes it — 588 of 622 now carry photo
URLs, 516 (83%) with the 2+ distinct images a merge requires.

*The `id_anchors.json` folklore, a third time: a state paragraph nobody
re-runs stops describing the world and starts describing the day it was
written.*
...
| route | strength | note |
|---|---|---|
| `ref` | **decisive WITHIN one agency only** (corrected S009) | their identifier, not our inference — but reference numbers are unique inside an agency, NOT across them. Measured S009: joining `agency_site_listings.ref` to `listings.agency_ref` yields 9 hits and all 9 are different properties (SICASA 0711 €95.000/100 m² against Centogambe 0711 €70.000/40 m²; Leonardi 6861 against Marcellini 6861). The valid use is an agency's own site against that agency's own portal rows — which is currently unavailable anyway, because `listings.agency_ref` is populated **0 of 844** for immobiliare |
